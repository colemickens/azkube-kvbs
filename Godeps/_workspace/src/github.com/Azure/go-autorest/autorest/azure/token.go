...
import (
	...
	"github.com/colemickens/azkvbs/Godeps/_workspace/src/github.com/dgrijalva/jwt-go"
	"github.com/colemickens/azkvbs/Godeps/_workspace/src/golang.org/x/crypto/pkcs12"
)
...
// ServicePrincipalCertificateSecret implements ServicePrincipalSecret for PKCS#12 cert auth with signed JWTs.
// deprecated!!!!!!!!!
...
// ServicePrincipalRsaSecret implements ServicePrincipalSecret for generic RSA cert auth with signed JWTs.
type ServicePrincipalRsaSecret struct {
	Certificate *x509.Certificate
	PrivateKey  *rsa.PrivateKey
}

// SignJwt returns the JWT signed with the certificate's private key.
func (secret *ServicePrincipalRsaSecret) SignJwt(spt *ServicePrincipalToken) (string, error) {
	hasher := sha1.New()
...
func (secret *ServicePrincipalRsaSecret) SetAuthenticationValues(spt *ServicePrincipalToken, v *url.Values) error {
...
func (secret *ServicePrincipalCertificateSecret) SetAuthenticationValues(spt *ServicePrincipalToken, v *url.Values) error {
	rsaPrivateKey, certificate, err := decodePkcs12(secret.Pkcs12, secret.Password)
	if err != nil {
		return nil
	}

	rsaSecret := &ServicePrincipalRsaSecret{
		Certificate: certificate,
		PrivateKey:  rsaPrivateKey,
	}

	return rsaSecret.SetAuthenticationValues(spt, v)
}
...
func NewServicePrincipalTokenFromCertificate(id string, pkcs12 []byte, password string, tenantID string, resource string) (*ServicePrincipalToken, error) {
	// this will only require us to parse the PKCS12 cert once
	// it also will inform the user they're doing something wrong sooner

	rsaPrivateKey, certificate, err := decodePkcs12(pkcs12, password)
	if err != nil {
		return nil, err
	}
...
// NewServicePrincipalTokenFromRsaKey create a ServicePrincipalToken from the supplied pkcs12 bytes.
func NewServicePrincipalTokenFromRsaKey(id string, certificate *x509.Certificate, privateKey *rsa.PrivateKey, tenantID string, resource string) (*ServicePrincipalToken, error) {
	return NewServicePrincipalTokenWithSecret(
		id,
		tenantID,
		resource,
...
func decodePkcs12(pkcs []byte, password string) (*rsa.PrivateKey, *x509.Certificate, error) {
	...
	if !isRsaKey {
		return nil, nil, fmt.Errorf("PKCS12 certificate must contain an RSA private key")
	}
